...
WildBench, which appeared as a spotlight paper at ICLR 2025, represents one of the most sophisticated implementations of this paradigm to date, and the key innovation that sets it apart is the use of task-specific checklists to guide the evaluation. The process works in three steps: first, you give the judge a real user prompt sourced from WildChat along with the target model's response; second, GPT-4-Turbo and Claude-3-Opus jointly generate a checklist of five to ten verifiable questions that are specifically tailored to that prompt, so for a coding task the checklist might ask "does the code handle edge cases?" while for an essay task it might ask "does the writing include the historical references the user requested?"; and third, the judge evaluates the response against this checklist using Chain-of-Thought reasoning, producing a structured analysis of strengths and weaknesses before arriving at a final score.

The importance of these checklists is backed by ablation results from the paper: without checklists, WB-Score achieves a Pearson correlation of 0.905 with human Elo ratings, but with checklists that number rises to 0.925, because the checklists reduce the subjectivity of evaluation by grounding it in concrete, task-specific criteria rather than leaving the judge to make vague holistic assessments.

---

## Slide 6: Two Metrics (1.5 min)

WildBench produces two complementary scores from this checklist-guided evaluation process, each designed for a different use case.

The first is WB-Score, which is a pointwise metric where the judge rates a single model's response on a scale from 1 to 10 against the checklist, with a clear rubric: scores of 1 to 2 mean the response makes no sense at all, 3 to 4 means it's poor and doesn't help the user, 5 to 6 means it's fair but has issues like factual errors or hallucinations, 7 to 8 means it's good but could be improved, and 9 to 10 means the response is perfect and fully solves the problem. The raw score is then rescaled using the formula S prime equals S minus 5 times 2, so that a score of 5, which represents a borderline acceptable response, maps to zero. WB-Score is fast and cheap, costing about five dollars and ten minutes per model, and it achieves a Pearson correlation of 0.955 with Arena Elo on the top-performing models.

The second metric is WB-Reward, which is a pairwise comparison where the judge compares the test model's response against a baseline model's response using the same checklist, producing one of five outcomes: much better, scored as plus one; slightly better, plus 0.5; tie, zero; slightly worse, minus 0.5; or much worse, minus one. To avoid the bias that comes from using a single baseline, WildBench averages the reward across three baselines at different performance tiers: GPT-4-Turbo-0429, Claude-3-Haiku, and Llama-2-70B-chat. It also applies a length penalty where if the winning response is more than K characters longer than the loser, with K set to 500 by default, a slight win gets converted to a tie, which mitigates the well-documented tendency of LLM judges to prefer longer responses. WB-Reward achieves a correlation of 0.984 with Arena Elo on top models.

---

## Slide 6b: Checklist Example (1 min)

To make this concrete, let me show you an actual example from the paper. One of the WildBench tasks is a prompt asking the model to write an essay on the impact of the G20 summit on the global economy, with specific constraints: it must be more than 1200 words, it should be beautiful and poetic with extensive vocabulary, and it should include ancient Indian historical references.

For this prompt, the system generates a checklist of six questions: does the essay contain more than 1200 words as requested? Is the language beautiful and poetic with extensive vocabulary? Does it include factual data on the G20's economic impact? Are there references to young people shaping the world? Does it include ancient Indian historical references? And is the essay structured in a clear and logical manner?

The crucial point is that every model's response to this same prompt gets evaluated against exactly the same checklist, which ensures that the scoring is consistent, interpretable, and directly comparable across all models. The checklists are generated once per task and then reused for every model being evaluated, so the evaluation criteria remain fixed even as you scale to 60 or more models.
